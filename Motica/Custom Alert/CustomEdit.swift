...
class CustomEdit: UIViewController {
    var task:[Task] = []
    ...
    override func viewDidLoad() {
        ...
        datePicker?.addTarget(self, action: #selector(CustomEdit.dateChanged(datePicker:)), for: .valueChanged)
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(CustomEdit.viewTapped(gestureRecognizer:)))
    ...
    @IBAction func onTapOkButton(_ sender: Any) {
        ...
        if (alertTextField.text! != ""){
            if(datePicker!.date >= dates as Date){
                self.task[row].title! = alertTextField.text!
                self.task[row].subtext! = NotesTextField.text!
                self.task[row].duedate! = datePicker!.date
